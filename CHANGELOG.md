...
- **Corrected scry parser package names** in `AGENT.md` and `CONFORMANCE.md`. `scry-parse-py` does not exist on PyPI (correct: `scry-parse`); `scry-parse-ts` does not exist on npm (correct: `@prmichaelsen/scry-parse`). Install commands and URLs updated accordingly.
...
- **`@acp.help` command** — structured catalog of all 48 ACP commands grouped by 8 categories (Workflow, Creation, Documentation, Clarification, Index, Package Management, Project Management, Version, Git Utilities). Includes one-line descriptions and a quick-start guide. Surfaced in AGENT.md and README as the entry point for command discoverability.
